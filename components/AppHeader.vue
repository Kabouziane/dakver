  ...
  <header class="sticky top-0 z-50 bg-white/95 backdrop-blur-sm shadow-sm border-b border-gray-100">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between h-16 md:h-20">
        ...
        <!-- Logo -->
        <NuxtLink to="/" class="flex items-center gap-2.5 group" aria-label="ToitureVerte - Accueil">
          ...
          <div>
            ...
            <p class="text-xs text-gray-500 -mt-0.5 hidden sm:block">Végétalisation · Belgique</p>
          </div>
        </NuxtLink>
        ...
        <nav class="hidden md:flex items-center gap-1" aria-label="Navigation principale">
          <NuxtLink
            ...
            :key="link.to"
            :to="link.to"
            class="px-4 py-2 text-sm font-medium text-gray-600 rounded-lg hover:text-brand-600 hover:bg-brand-50 transition-colors"
        ...
        <div class="flex items-center gap-3">
          <NuxtLink to="/contact" class="hidden sm:inline-flex btn-primary text-sm py-2.5">
            Devis gratuit
          </NuxtLink>
    ...
    >
      <nav v-if="mobileOpen" class="md:hidden border-t border-gray-100 bg-white px-4 py-3 space-y-1" aria-label="Navigation mobile">
        <NuxtLink
          v-for="link in navLinks"
          :key="link.to"
          :to="link.to"
        ...
        <div class="pt-2 border-t border-gray-100">
          <NuxtLink to="/contact" class="btn-primary w-full text-sm py-3" @click="mobileOpen = false">
            Devis gratuit
          </NuxtLink>
        </div>
      </nav>
